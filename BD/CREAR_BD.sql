...
CREATE TABLE DETALLE_TRANSPORTISTA (ID_DETALLE_TRANSPORTISTA INT UNIQUE AUTO_INCREMENT,ID_TRANSPORTISTA INT, METODO_VIAJE VARCHAR(3) NOT NULL,
REFRIGERADO BOOLEAN NOT NULL, TON_MAX FLOAT(6,3) NOT NULL,PRECIO_KM FLOAT(6,2), DESCRIPCION VARCHAR(150),PRIMARY KEY(ID_TRANSPORTISTA,METODO_VIAJE,REFRIGERADO));
...
CREATE TABLE PERSONA (ID_USUARIO INT, RUT VARCHAR(8) PRIMARY KEY, DIGITO_VERIFICADOR VARCHAR(1) NOT NULL, NOMBRE VARCHAR(100) NOT NULL, 
	APELLIDO VARCHAR(100), NOMBRE_FANTASIA VARCHAR(100), CODIGO_POSTAL INT, ID_COMUNA INT NOT NULL, TELEFONO INT, ID_TIPO_PERSONA_LEGAL INT NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE PEDIDO (ID_PEDIDO INT PRIMARY KEY AUTO_INCREMENT, ID_VENDEDOR INT,ID_COMPRADOR INT NOT NULL,ID_TIPO_PEDIDO INT NOT NULL, ID_ESTADO_PEDIDO INT NOT NULL,
	FECHA_CREACION DATETIME NOT NULL, FECHA_LIMITE_O_RETIRO DATE, FECHA_PAGO DATETIME);
CREATE TABLE DETALLE_PEDIDO (ID_DETALLE_PEDIDO INT PRIMARY KEY AUTO_INCREMENT,ID_PEDIDO INT , ID_TIPO_FRUTA INT, ID_CALIDAD INT,METODO_VIAJE VARCHAR(3), REFRIGERADO tinyint(1), CANT_KG FLOAT(6,3), PRECIO_KG FLOAT(9,2), COD_MONEDA VARCHAR(3));
...
CREATE TABLE POSTULACION (ID_POSTULACION INT PRIMARY KEY AUTO_INCREMENT, ID_DETALLE_PEDIDO INT NOT NULL, ID_TIPO_POSTULACION INT NOT NULL, ID_USUARIO INT NOT NULL, 
	ID_ESTADO INT NOT NULL, KG_APORTADOS FLOAT(6,3) NOT NULL,PRECIO FLOAT(9,2));
CREATE TABLE DESPACHO (ID_DESPACHO INT PRIMARY KEY AUTO_INCREMENT, ID_PEDIDO INT NOT NULL, ID_TRANSPORTISTA INT NOT NULL,
	ID_ESTADO_DESPACHO INT NOT NULL, FECHA_DESPACHO DATETIME, FECHA_RECIBIDO DATETIME);
CREATE TABLE DETALLE_DESPACHO (ID_DESPACHO INT, FECHA_ACTUALIZACION DATETIME, ID_TIPO_ACTUALIZACION INT NOT NULL, DESCRIPCION VARCHAR(150),
	PRIMARY KEY(ID_DESPACHO,FECHA_ACTUALIZACION));
CREATE TABLE HISTORICO_TRANSACCIONES (ID_TRANSACCION INT AUTO_INCREMENT, FECHA_TRANSACCION DATETIME, CORREO_COMPRADOR VARCHAR (100), CORREO_VENDEDOR VARCHAR (100), NOMBRE_FRUTA VARCHAR (100), 
TIPO_CALIDAD VARCHAR (100), SUBTOTAL_TRANSACCION FLOAT(9,2),
...
ADD FOREIGN KEY (ID_DETALLE_PEDIDO) REFERENCES DETALLE_PEDIDO(ID_DETALLE_PEDIDO);
ALTER TABLE POSTULACION
...
ALTER TABLE DETALLE_TRANSPORTISTA
ADD FOREIGN KEY (ID_TRANSPORTISTA) REFERENCES USUARIO(ID_USUARIO);