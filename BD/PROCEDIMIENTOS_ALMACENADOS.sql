...
BEGIN
	...
	IF ($ID_PERFIL = 2 || $ID_PERFIL = 5) THEN  
		...
		CAST(SYSDATE() AS DATE) + INTERVAL 1 YEAR);
	END IF;
END //
DELIMITER ;
...
#PROCEDIMIENTO PARA INSERTAR UNA NUEVA POSTULACION A TRANSPORTISTA
DROP PROCEDURE IF EXISTS SP_CREATE_POSTULACION_TRANSPORTISTA;
DELIMITER //
CREATE PROCEDURE SP_CREATE_POSTULACION_TRANSPORTISTA(
	IN $ID_DETALLE_PEDIDO INT,
	IN $ID_USUARIO VARCHAR(100),
    OUT $RESULTADO INT
)
BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION 
    ...
    START TRANSACTION;
        #SE INSERTA EL NUEVO PEDIDO CON ESTADO INICIAL
		INSERT INTO `maipo_grande`.`postulacion` (ID_DETALLE_PEDIDO,ID_TIPO_POSTULACION,ID_USUARIO,ID_ESTADO)
        VALUES
        ($ID_DETALLE_PEDIDO,2,$ID_USUARIO,(SELECT ID_ESTADO  FROM `maipo_grande`.`estados` WHERE NOMBRE = 'POSTULADO' LIMIT 1));
        #OBTENER ID DE POSTULACION GENERADOS
        SELECT MAX(ID_POSTULACION) INTO $RESULTADO FROM `maipo_grande`.`postulacion`;
        COMMIT;
END //
...
BEGIN
    ...
    #SE ELIMINA HISTORICO STOCK ASOCIADO
    DELETE
...
#SE ELIMINAN LAS POSTULACION ASOCIADAS A ESE TIPO DE FRUTA
DELETE FROM POSTULACION WHERE ID_DETALLE_PEDIDO IN (SELECT ID_DETALLE_PEDIDO FROM DETALLE_PEDIDO WHERE ID_TIPO_FRUTA = $ID_TIPO_FRUTA);
...
IN $TON_MAX FLOAT(12,3),
IN $PRECIO_KM FLOAT(12,2),
IN $DESCRIPCION VARCHAR(150)
)
BEGIN
...
DROP PROCEDURE IF EXISTS SP_GET_TRANSPORTISTAS;
DELIMITER //
...
#PROCEDIMIENTO PARA DECIDIR GANADOR DE POSTULACIONES DE TRANSPORTISTA
DROP PROCEDURE IF EXISTS SP_FINALIZACION_POSTULACION_TRANSPORTISTA;
DELIMITER //
CREATE PROCEDURE SP_FINALIZACION_POSTULACION_TRANSPORTISTA(
IN $ID_PEDIDO INT,
#0:NO HAY POSTULACIONES SUFICIENTES |1: CORRECTO | NULL: ERROR EN EL PROCEDIMIENTO
OUT $RES INT
)
BEGIN
    ...
    WHERE DP.ID_PEDIDO = $ID_PEDIDO AND ID_TIPO_POSTULACION = 2) > 0 THEN
		#SE COMIENZA PROCESO DE ASIGNACION DE POSTULACIONES
		SELECT ID_POSTULACION,P.ID_DETALLE_PEDIDO,ID_TRANSPORTISTA #INTO $ID_POSTULACION,$ID_DETALLE_PEDIDO,$ID_TRANSPORTISTA 
        FROM POSTULACION P 
        JOIN DETALLE_PEDIDO DP ON DP.ID_DETALLE_PEDIDO = P.ID_DETALLE_PEDIDO
        JOIN DETALLE_TRANSPORTISTA DT ON DT.ID_TRANSPORTISTA = P.ID_USUARIO
        WHERE DP.ID_PEDIDO = $ID_PEDIDO 
        AND DT.METODO_VIAJE =DP.METODO_VIAJE
        ORDER BY DT.PRECIO_KM 
        LIMIT 1;				
		UPDATE `maipo_grande`.`postulacion` SET `ID_ESTADO` = (SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'APROBADA') WHERE `ID_POSTULACION` = $ID_POSTULACION;
        UPDATE `maipo_grande`.`postulacion` SET `ID_ESTADO` = (SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'RECHAZADO') WHERE `ID_DETALLE_PEDIDO` = $ID_DETALLE_PEDIDO  AND `ID_ESTADO` = (SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'POSTULADO');
        UPDATE `maipo_grande`.`pedido` SET `ID_ESTADO` = (SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'DESPACHO') WHERE ID_PEDIDO = $ID_PEDIDO;
        INSERT INTO `maipo_grande`.`despacho`(`ID_PEDIDO`,`ID_TRANSPORTISTA`,`ID_ESTADO_DESPACHO`,`FECHA_DESPACHO`)VALUES($ID_PEDIDO,$ID_TRANSPORTISTA,(SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'DESPACHO'),SYSDATE());
        SELECT MAX(ID_DESPACHO) INTO $ID_DESPACHO FROM DESPACHO;
        INSERT INTO `maipo_grande`.`detalle_despacho`(`ID_DESPACHO`,`FECHA_ACTUALIZACION`,`ID_TIPO_ACTUALIZACION`,`DESCRIPCION`)VALUES($ID_DESPACHO,SYSDATE(),(SELECT ID_ESTADO FROM ESTADOS WHERE NOMBRE = 'INFORMATIVA'), 'Se ha asignado un transportista, y se da inicio al proceso de despacho');
	COMMIT;
	END IF;
END//
DELIMITER ;

#PROCEDIMIENTO QUE TRAE LAS VENTAS EXTERNAS DISPONIBLES PARA QUE EL TRANSPORTISTA POSTULE
DROP PROCEDURE IF EXISTS SP_GET_VENTAS_EXTERNAS_TRANSPORTE;
DELIMITER //
CREATE PROCEDURE `SP_GET_VENTAS_EXTERNAS_TRANSPORTE`(
IN $CORREO VARCHAR(100)
)
BEGIN
	SELECT 
	L.ID_PEDIDO,
	REPLACE(REPLACE(REPLACE(L.METODO_VIAJE,'ear','Terrestre'),'sea','Maritimo'),'air','Aereo') METODO_VIAJE,
	(CASE
	WHEN L.REFRIGERADO = 1 THEN
		'Si'
	ELSE
		'No'
	END) REFRIGERADO,
	...
	L.ESTADO,
	L.CORREO_COMPRADOR
	FROM DETALLE_TRANSPORTISTA D,
	(SELECT DISTINCT P.ID_PEDIDO,
	DP.METODO_VIAJE,
	(CASE
	WHEN P.ID_PEDIDO IN (SELECT ID_PEDIDO
	FROM DETALLE_PEDIDO DP
	WHERE REFRIGERADO = 1) THEN
		1
	ELSE
		0
	END) REFRIGERADO,
	SUM(DP.CANT_KG) CANT_KG_TOTAL,
	E.NOMBRE ESTADO,
	U.CORREO CORREO_COMPRADOR
	FROM PEDIDO P
	JOIN USUARIO U ON U.ID_USUARIO = P.ID_COMPRADOR
	JOIN ESTADOS E ON E.ID_ESTADO = P.ID_ESTADO_PEDIDO
	JOIN DETALLE_PEDIDO DP ON DP.ID_PEDIDO = P.ID_PEDIDO
	WHERE P.ID_PEDIDO NOT IN (SELECT DP.ID_PEDIDO
					FROM POSTULACION P
					JOIN DETALLE_PEDIDO DP ON DP.ID_DETALLE_PEDIDO = P.ID_DETALLE_PEDIDO
					JOIN ESTADOS E ON E.ID_ESTADO = P.ID_ESTADO
					JOIN USUARIO U ON U.ID_USUARIO = P.ID_USUARIO
					WHERE U.CORREO = $CORREO)
	AND P.ID_TIPO_PEDIDO = 2
	GROUP BY DP.ID_PEDIDO,DP.METODO_VIAJE
	HAVING E.NOMBRE = 'EN LOGISTICA') L
	WHERE D.ID_TRANSPORTISTA = (SELECT ID_USUARIO FROM USUARIO WHERE CORREO = $CORREO)
	AND D.TON_MAX >= L.CANT_KG_TOTAL
	AND D.METODO_VIAJE = L.METODO_VIAJE;
END //
DELIMITER ;

DROP PROCEDURE IF EXISTS SP_GET_VENTAS_EXTERNAS_HISTORICO;
DELIMITER //
CREATE PROCEDURE `SP_GET_VENTAS_EXTERNAS_HISTORICO` (
IN $CORREO VARCHAR(100)
)
BEGIN
SELECT L.*,E.NOMBRE ESTADO
FROM POSTULACION PO
JOIN ESTADOS E ON E.ID_ESTADO = PO.ID_ESTADO
JOIN USUARIO UV ON UV.ID_USUARIO = PO.ID_USUARIO
JOIN (SELECT 
			PE.ID_PEDIDO, DP.ID_DETALLE_PEDIDO,
			REPLACE(REPLACE(REPLACE(DP.METODO_VIAJE,'ear','Terrestre'),'sea','Maritimo'),'air','Aereo') METODO_VIAJE,
			(CASE
			WHEN PE.ID_PEDIDO IN (SELECT ID_PEDIDO
			FROM DETALLE_PEDIDO DP
			WHERE REFRIGERADO = 1) THEN
				1
			ELSE
			...
			END) REFRIGERADO,
			SUM(DP.CANT_KG) CANT_KG_TOTAL,
			UC.CORREO CORREO_COMPRADOR
			FROM PEDIDO PE
			JOIN DETALLE_PEDIDO DP ON DP.ID_PEDIDO = PE.ID_PEDIDO
			JOIN USUARIO UC ON UC.ID_USUARIO = PE.ID_COMPRADOR
			GROUP BY DP.ID_PEDIDO,PE.ID_COMPRADOR) L ON L.ID_DETALLE_PEDIDO = PO.ID_DETALLE_PEDIDO
WHERE UV.CORREO = $CORREO
AND E.NOMBRE IN ('RECHAZADO','APROBADA');
END //
DELIMITER ;

DROP PROCEDURE IF EXISTS SP_GET_VENTAS_EXT_POSTULADAS_TR;
DELIMITER //
CREATE PROCEDURE `SP_GET_VENTAS_EXT_POSTULADAS_TR` (
IN $CORREO VARCHAR(100))
BEGIN
SELECT L.*,E.NOMBRE ESTADO
FROM POSTULACION PO
...
JOIN (SELECT 
			...
			GROUP BY DP.ID_PEDIDO,PE.ID_COMPRADOR) L ON L.ID_DETALLE_PEDIDO = PO.ID_DETALLE_PEDIDO
WHERE UV.CORREO = $CORREO
AND E.NOMBRE IN ('POSTULADO');
END //
DELIMITER ;


DROP PROCEDURE IF EXISTS SP_MAIL_VENTA_INTERNA;
DELIMITER //
CREATE PROCEDURE `SP_MAIL_VENTA_INTERNA`(IN $idpedido INT)
BEGIN
SELECT P.ID_PEDIDO, U.CORREO CORREO_COMPRADOR,UU.CORREO CORREO_VENDEDOR, 
P.FECHA_CREACION,TF.NOMBRE NOMBRE_FRUTA, C.NOMBRE NOMBRE_CALIDAD, D.CANT_KG CANTIDAD_COMPRADA
FROM PEDIDO P 
JOIN DETALLE_PEDIDO D ON P.ID_PEDIDO = D.ID_PEDIDO 
JOIN USUARIO U ON P.ID_COMPRADOR = U.ID_USUARIO
JOIN USUARIO UU ON D.ID_VENDEDOR = UU.ID_USUARIO
JOIN TIPO_FRUTA TF ON TF.ID_TIPO_FRUTA = D.ID_TIPO_FRUTA
JOIN CALIDAD C ON D.ID_CALIDAD = C.ID_CALIDAD
