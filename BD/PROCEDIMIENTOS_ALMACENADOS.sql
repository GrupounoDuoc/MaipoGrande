...
BEGIN
    ...
    #SE INSERTAN LOS PRODUCTOS DEL CARRO
    DECLARE curDetallePedido
		CURSOR FOR 
			SELECT JSON_EXTRACT($ID_STOCK, '$.id') ID;
	#declare NOT FOUND handler
	DECLARE CONTINUE HANDLER 
        FOR NOT FOUND SET finished = 1;
        
    #SE OBTIENE EL ID CORRESPONDIENTE AL TIPO DE FRUTA
    SELECT ID_TIPO_FRUTA INTO $ID_TIPO_FRUTA FROM `maipo_grande`.`tipo_fruta` WHERE NOMBRE=$TIPO_FRUTA;
    #SE OBTIENE EL ID CORRESPONDIENTE A LA CALIDAD
    SELECT ID_CALIDAD INTO $ID_CALIDAD FROM `maipo_grande`.`calidad` WHERE NOMBRE=$CALIDAD;
    #SE OBTIENE EL PRECIO DEL PEDIDO DESDE EL HISTORICO
    SELECT PRECIO_X_KG INTO $PRECIO FROM `maipo_grande`.`historico_stock` WHERE ID_STOCK=$ID_STOCK;
	...
	insertDetalle: LOOP
		FETCH curDetallePedido INTO stock;
		IF finished = 1 THEN 
			LEAVE insertDetalle;
		END IF;
		INSERT INTO `maipo_grande`.`detalle_pedido`(`ID_PEDIDO`,`ID_TIPO_FRUTA`,`ID_CALIDAD`,`CANT_KG`,`PRECIO_KG`,`COD_MONEDA`)VALUES($ID_PEDIDO,$ID_TIPO_FRUTA,$ID_CALIDAD,$CANT,$PRECIO,'CLP');
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CREATE_TIPO_FRUTA`(
IN $NOMBRE VARCHAR(100),
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DELETE_TIPO_FRUTA`($ID_TIPO_FRUTA INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UPDATE_TIPO_FRUTA`(
$ID_TIPO_FRUTA INT,
$NOMBRE VARCHAR(100),
