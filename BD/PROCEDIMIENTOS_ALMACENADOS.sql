    ...
    START TRANSACTION;
		...
		insertDetalle: LOOP
			IF $COUNTER > JSON_LENGTH($JSON) THEN 
				LEAVE insertDetalle;
			END IF;
			select HS.ID_TIPO_FRUTA,HS.ID_CALIDAD,replace(JSON_EXTRACT($JSON, CONCAT('$.',$counter,'.cantidad')),'"',''),HS.PRECIO_X_KG , TF.NOMBRE, C.NOMBRE, U.CORREO
				INTO $ID_TIPO_FRUTA,$ID_CALIDAD,$CANTIDAD,$PRECIO,$NOMBRE_FRUTA,$NOMBRE_CALIDAD,$CORREO_VENDEDOR
