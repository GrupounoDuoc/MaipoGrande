...
BEGIN
    ...
    START TRANSACTION;
        ...
        VALUES
        ($ID_DETALLE_PEDIDO,$TIPO_POSTULACION,$ID_USUARIO,$ID_ESTADO);
        #OBTENER ID DE POSTULACION GENERADOS
        SELECT MAX(ID_POSTULACION) INTO $RESULTADO FROM `maipo_grande`.`postulacion`;
        COMMIT;
END //
...
AND (SELECT MAX(REFRIGERADO) FROM DETALLE_TRANSPORTISTA DT JOIN USUARIO U ON U.ID_USUARIO=DT.ID_TRANSPORTISTA WHERE CORREO = 'transportista@mail.cl') = L.REFRIGERADO;
END //
DELIMITER ;
